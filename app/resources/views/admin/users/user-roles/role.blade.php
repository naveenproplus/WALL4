						...
						<div class="card-body">
							...
							<div class="row  mt-20 ml-20">
								<div class="col-sm-12 text-right">
									@if($crud['view']==1)
										<a id="btnCancel" href="{{ url('/') }}/admin/users-and-permissions/user-roles/" class="btn btn-sm  btn-outline-light  active txt-dark mr-10">Cancel</a>
									@endif
    ...
    $(document).ready(function(){
		...
		$('#btnCancel').click(()=>{
			if(blnProcessing==false){
				@if($crud['view']==1)
					window.location.replace("{{url('/')}}/admin/users-and-permissions/user-roles/view");
				@endif
			}else{
		...
		const Get_User_Role_info=async()=>{
			...
			@if($isEdit==true)
				...
				xhttp.open("POST", "{{ url('/') }}/admin/users-and-permissions/user-roles/json/"+RoleID, true);
				xhttp.send(formData);
